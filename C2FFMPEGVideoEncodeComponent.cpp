...
#define LOG_TAG "C2FFMPEGVideoEncodeComponent"
...
C2FFMPEGVideoEncodeComponent::C2FFMPEGVideoEncodeComponent(
        const C2FFMPEGComponentInfo* componentInfo,
        const std::shared_ptr<C2FFMPEGVideoEncodeInterface>& intf)
    : SimpleC2Component(std::make_shared<SimpleInterface<C2FFMPEGVideoEncodeInterface>>(componentInfo->name, 0, intf)),
      mInfo(componentInfo),
      mIntf(intf),
      ...
      mEOSSignalled(false) {
    ALOGD("C2FFMPEGVideoEncodeComponent: mediaType = %s", componentInfo->mediaType);
}

C2FFMPEGVideoEncodeComponent::~C2FFMPEGVideoEncodeComponent() {
    ALOGD("~C2FFMPEGVideoEncodeComponent: mCtx = %p", mCtx);
    onRelease();
}

c2_status_t C2FFMPEGVideoEncodeComponent::initEncoder() {
    if (! mFFMPEGInitialized) {
        if (initFFmpeg() != C2_OK) {
            ALOGE("initEncoder: FFMPEG initialization failed.");
            return C2_NO_INIT;
        }
    ...
    if (! mCtx) {
        ALOGE("initEncoder: avcodec_alloc_context failed.");
        return C2_NO_MEMORY;
    }
    ...
    if (err != C2_OK) {
        ALOGE("initEncoder: cannot query picture size, err = %d", err);
    }
    ...
    ALOGD("initEncoder: %p [%s], %d x %d, %s",
          mCtx, avcodec_get_name(mCtx->codec_id), size.width, size.height, mInfo->mediaType);

    return C2_OK;
}

c2_status_t C2FFMPEGVideoEncodeComponent::openEncoder() {
    if (mCodecAlreadyOpened) {
        return C2_OK;
    }

    // Can't change extradata after opening the encoder.
#if DEBUG_EXTRADATA
    ALOGD("openEncoder: extradata_size = %d", mCtx->extradata_size);
#endif
    mExtradataReady = true;

    // Find encoder again as codec_id may have changed.
    if (mCtx->codec_id == AV_CODEC_ID_H264 &&
            base::GetBoolProperty("vendor.ffmpeg_codec2.v4l2.h264", false)) {
        mCtx->codec = avcodec_find_encoder_by_name("h264_v4l2m2m");
    } else {
        mCtx->codec = avcodec_find_encoder(mCtx->codec_id);
    }

    if (! mCtx->codec) {
        ALOGE("openEncoder: ffmpeg video encoder failed to find codec %d", mCtx->codec_id);
        return C2_NOT_FOUND;
    }
    ...
    ALOGD("openEncoder: opening ffmpeg encoder(%s): threads = %d, hw = %s",
          avcodec_get_name(mCtx->codec_id), mCtx->thread_count, mCtx->hw_device_ctx ? "yes" : "no");

    int err = avcodec_open2(mCtx, mCtx->codec, NULL);
    if (err < 0) {
        ALOGE("openEncoder: ffmpeg video encoder failed to initialize. (%s)", av_err2str(err));
        return C2_NO_INIT;
    }
    mCodecAlreadyOpened = true;

    ALOGD("openEncoder: open ffmpeg video encoder(%s) success, caps = %08x",
          avcodec_get_name(mCtx->codec_id), mCtx->codec->capabilities);

    mFrame = av_frame_alloc();
    if (! mFrame) {
        ALOGE("openEncoder: oom for video frame");
        return C2_NO_MEMORY;
    }
...
void C2FFMPEGVideoEncodeComponent::deInitEncoder() {
    ALOGD("%p deInitEncoder: %p", this, mCtx);
    if (mCtx) {
...
c2_status_t C2FFMPEGVideoEncodeComponent::processCodecConfig(C2ReadView* inBuffer) {
    ...
    if (! mExtradataReady) {
        ...
        if (! mCtx->extradata) {
            ALOGE("processCodecConfig: ffmpeg video encoder failed to alloc extradata memory.");
            return C2_NO_MEMORY;
        }
        ...
        ALOGW("processCodecConfig: encoder is already opened, ignoring...");
    }

    return C2_OK;
}

c2_status_t C2FFMPEGVideoEncodeComponent::sendInputBuffer(
    ...
    if (err < 0) {
        ALOGE("sendInputBuffer: failed to send data (%d) to encoder: %s (%08x)",
              inBuffer->capacity(), av_err2str(err), err);
        if (err == AVERROR(EAGAIN)) {
            // Frames must be read first, notify main encoding loop.
...
c2_status_t C2FFMPEGVideoEncodeComponent::receiveFrame(bool* hasPicture) {
    ...
    if (err == 0) {
        ...
        if (err == 0) {
            *hasPicture = true;
            ...
            ALOGE("receiveFrame: failed to receive frame from HW encoder err = %d", err);
            // Don't send error to client, skip frame!
        }
    } else if (err != AVERROR(EAGAIN) && err != AVERROR_EOF) {
        ALOGE("receiveFrame: failed to receive frame from encoder err = %d", err);
...
c2_status_t C2FFMPEGVideoEncodeComponent::getOutputBuffer(C2GraphicView* outBuffer) {
    uint8_t* data[4];
    int linesize[4];
...
void C2FFMPEGVideoEncodeComponent::pushPendingWork(const std::unique_ptr<C2Work>& work) {
...
void C2FFMPEGVideoEncodeComponent::popPendingWork(const std::unique_ptr<C2Work>& work) {
...
void C2FFMPEGVideoEncodeComponent::prunePendingWorksUntil(const std::unique_ptr<C2Work>& work) {
#if DEBUG_WORKQUEUE
...
c2_status_t C2FFMPEGVideoEncodeComponent::onInit() {
    ALOGD("onInit");
    return initEncoder();
}

c2_status_t C2FFMPEGVideoEncodeComponent::onStop() {
    ALOGD("onStop");
    return C2_OK;
}

void C2FFMPEGVideoEncodeComponent::onReset() {
    ALOGD("onReset");
    deInitEncoder();
    initEncoder();
}

void C2FFMPEGVideoEncodeComponent::onRelease() {
    ALOGD("onRelease");
    deInitEncoder();
    if (mFFMPEGInitialized) {
        deInitFFmpeg();
...
c2_status_t C2FFMPEGVideoEncodeComponent::onFlush_sm() {
    ALOGD("onFlush_sm");
    if (mCtx && avcodec_is_open(mCtx)) {
        // Make sure that the next buffer output does not still
        // depend on fragments from the last one encoded.
        avcodec_flush_buffers(mCtx);
        mEOSSignalled = false;
    }
    return C2_OK;
...
c2_status_t C2FFMPEGVideoEncodeComponent::outputFrame(
...
void C2FFMPEGVideoEncodeComponent::process(
    const std::unique_ptr<C2Work> &work,
    const std::shared_ptr<C2BlockPool> &pool
) {
    size_t inSize = 0u;
    ...
    // in and stalls the encoder. But there's no obvious mechanism to determine, from
...
c2_status_t C2FFMPEGVideoEncodeComponent::drain(
    uint32_t drainMode,
    const std::shared_ptr<C2BlockPool>& pool
) {
    ...
    while (err == C2_OK) {
        ...
        err = receiveFrame(&hasPicture);
        if (hasPicture) {
            // Ignore errors at this point, just drain the encoder.
